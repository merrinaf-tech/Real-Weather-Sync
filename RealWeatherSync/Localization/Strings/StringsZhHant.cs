namespace RealWeatherSync.Localization.Strings
{
    ...
    public static class StringsZhHant
    {
        ...
        public static readonly string[,] Table =
        {
            ...
            // -- Advanced -----------------------------------------------------------
            { "label.SyncAurora", "同步 NOAA 極光預報" },
            { "desc.SyncAurora", "選用。使用天氣地點的 NOAA 最新極光預報。僅在當地與遊戲中皆為夜晚時顯示。跟隨遊戲時鐘時仍維持此行為；僅在手動時間偏移時無法使用。預報不保證能觀測到極光。" },
            { "label.SyncSunPosition", "同步太陽的真實位置" },
            { "desc.SyncSunPosition", "選用，預設關閉。將畫面中的太陽放在天氣地點此刻所見的真實位置。日光與陰影會隨之變化，但遊戲時鐘、日期、季節及存檔不會改變。跟隨遊戲時鐘與時間偏移仍只影響天氣。需要啟用遊戲的晝夜視覺效果。" },
            { "label.SyncFog", "同步霧" },
            { "desc.SyncFog",
            ...
            { "label.LimitWeatherEvents", "僅在真實雷暴時允許天氣事件" },
            { "desc.LimitWeatherEvents",
                "預設關閉。當真實天氣驅動你的城市時，暫停遊戲的天氣事件生成（龍捲風、冰雹、雷擊），除非真實城市確實正在打雷暴。遊戲僅依據氣溫、降雨與雲量來選擇這些事件，因此若不開啟此項，平靜的真實天氣也可能颳出龍捲風。它只是暫停產生器，不會建立任何東西：不會寫入任何內容到存檔，關閉後遊戲會立刻收回控制權。開啟期間，遊戲原本會自行產生的事件同樣被暫停，兩者無法區分。" },
            ...
            { "key.About.Text",
                ...
                "選用的極光同步會下載 NOAA 全球預報，不會傳送你的所在位置。" },
