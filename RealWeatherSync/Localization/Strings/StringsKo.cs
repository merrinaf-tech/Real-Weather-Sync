namespace RealWeatherSync.Localization.Strings
{
    ...
    public static class StringsKo
    {
        ...
        public static readonly string[,] Table =
        {
            ...
            // -- General ------------------------------------------------------------
            { "label.EnableRealWeather", "Real Weather Sync 사용" },
            ...
            { "label.FollowGameClock", "최근 24시간을 게임 내 시계와 동기화" },
            ...
            { "desc.SyncAurora", "선택 사항입니다. 날씨 위치의 NOAA 최신 오로라 예보를 사용합니다. 해당 지역과 게임이 모두 밤일 때만 표시됩니다. 게임 시계 따라가기 모드에서도 이 동작을 유지하며 수동 시간 이동 시에만 사용할 수 없습니다. 예보가 실제 관측을 보장하지는 않습니다." },
            { "label.SyncSunPosition", "실제 태양 위치 동기화" },
            { "desc.SyncSunPosition", "선택 사항이며 기본값은 꺼짐입니다. 표시되는 태양을 날씨 위치에서 본 현재의 실제 위치에 놓습니다. 햇빛과 그림자는 이를 따르지만 게임 시계, 날짜, 계절 및 저장 데이터는 바뀌지 않습니다. 게임 시계 따라가기와 시간 이동은 계속 날씨에만 영향을 줍니다. 게임의 낮/밤 표시가 필요합니다." },
            ...
            { "label.LimitWeatherEvents", "기상 이벤트를 실제 뇌우일 때로 제한" },
            { "desc.LimitWeatherEvents",
                "기본적으로 꺼져 있습니다. 실제 날씨가 도시를 제어하는 동안 게임의 기상 이벤트 생성(토네이도, 우박 폭풍, 낙뢰)을 막습니다. 다만 실제 도시에 정말로 뇌우가 있을 때는 막지 않습니다. 게임은 이 이벤트를 기온, 비, 구름량만으로 결정하므로 이 설정이 없으면 잔잔한 실제 날씨에도 토네이도가 생길 수 있습니다. 생성기를 잠시 멈출 뿐 아무것도 만들지 않으므로 저장 파일에는 아무것도 기록되지 않으며, 끄면 즉시 게임에 돌려줍니다. 켜져 있는 동안에는 게임이 스스로 만들었을 이벤트도 함께 막힙니다. 둘을 구별할 방법이 없습니다." },
