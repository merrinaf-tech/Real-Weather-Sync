namespace RealWeatherSync.Localization.Strings
{
    ...
    public static class StringsZhHans
    {
        ...
        public static readonly string[,] Table =
        {
            ...
            // -- Advanced -----------------------------------------------------------
            { "label.SyncAurora", "同步 NOAA 极光预报" },
            { "desc.SyncAurora", "可选。使用天气地点的 NOAA 最新极光预报。仅当当地和游戏中都处于夜晚时显示。跟随游戏时钟时仍保持此行为；仅在手动时间偏移时不可用。预报不保证能够观测到极光。" },
            { "label.SyncSunPosition", "同步太阳的真实位置" },
            { "desc.SyncSunPosition", "可选，默认关闭。将画面中的太阳放在天气地点此刻看到的真实位置。日光和阴影会随之变化，但游戏时钟、日期、季节和存档不会改变。跟随游戏时钟和时间偏移仍只影响天气。需要启用游戏的昼夜视觉效果。" },
            { "label.SyncFog", "同步雾" },
            { "desc.SyncFog",
            ...
            { "label.LimitWeatherEvents", "仅在真实雷暴时允许天气事件" },
            { "desc.LimitWeatherEvents",
                "默认关闭。当真实天气驱动你的城市时，暂停游戏的天气事件生成（龙卷风、冰雹、雷击），除非真实城市确实正在打雷暴。游戏仅根据气温、降雨和云量来选择这些事件，因此若不开启此项，平静的真实天气也可能刮出龙卷风。它只是暂停生成器，不会创建任何东西：不会向存档写入任何内容，关闭后游戏立刻收回控制权。开启期间，游戏本来会自行产生的事件同样被暂停，两者无法区分。" },
            ...
            { "key.About.Text",
                ...
                "可选极光同步会下载 NOAA 全球预报，不会发送你的位置信息。" },
