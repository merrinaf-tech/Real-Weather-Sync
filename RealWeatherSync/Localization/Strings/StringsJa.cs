namespace RealWeatherSync.Localization.Strings
{
    ...
    public static class StringsJa
    {
        ...
        public static readonly string[,] Table =
        {
            ...
            // -- General ------------------------------------------------------------
            { "label.EnableRealWeather", "Real Weather Sync を有効にする" },
            ...
            { "label.FollowGameClock", "直近24時間をゲーム内時計と同期する" },
            ...
            // -- Advanced -----------------------------------------------------------
            { "label.SyncAurora", "NOAAのオーロラ予報と同期" },
            { "desc.SyncAurora", "任意。天気の参照地点に対するNOAAの最新オーロラ予報を使用します。現地とゲーム内の両方が夜の場合のみ表示します。ゲーム内時計に従うモードでもこの動作を維持し、手動の時刻シフト時のみ利用できません。予報は観測を保証しません。" },
            { "label.SyncSunPosition", "太陽の実際の位置を同期" },
            { "desc.SyncSunPosition", "任意で、初期設定はオフです。表示される太陽を、天気の場所から見た現在の実際の位置に合わせます。日光と影は追従しますが、ゲーム内時計、日付、季節、セーブデータは変更しません。ゲーム内時計追従と時間シフトは引き続き天気だけに影響します。ゲームの昼夜表示が必要です。" },
            ...
            { "label.LimitWeatherEvents", "気象イベントを実際の雷雨のときだけに制限する" },
            { "desc.LimitWeatherEvents",
                "既定ではオフです。実際の天気が都市を制御している間、ゲームの気象イベント生成（竜巻、雹嵐、落雷）を抑えます。ただし実際の都市が本当に雷雨のときは抑えません。ゲームはこれらのイベントを気温・雨・雲量だけから決めるため、この設定がないと穏やかな実際の天気でも竜巻が起こり得ます。生成を一時停止するだけで何も作成しないので、セーブデータには何も書き込まれず、オフにすればすぐにゲームへ戻ります。有効な間は、ゲームが自力で起こしていたはずのイベントも抑えられます。区別する方法はありません。" },
            ...
            { "key.About.Text",
                ...
                "任意のオーロラ同期では、位置情報を送信せずにNOAAの全球予報を取得します。" },
